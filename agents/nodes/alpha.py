...
# 시스템 프롬프트
# ==========================================
ALPHA_SYSTEM_PROMPT = """너는 AlphaInvest 서비스에서 소비자에게 제공되는
...
3. 종목 제시 방식:
   ...
   - "방어주", "성장주" 같은 포괄적 표현 대신, 반드시 "헬스케어", "전력 인프라", "필수소비재", "반도체"처럼 구체적인 산업 섹터명으로 작성한다.
...
5. 금지 사항:
   ...
   6. 추가 작성 규칙
- 각 섹터마다 미국/한국 대표 종목은 최소 2개씩 제시하되, 실제 시장에서 널리 인정되는 대형주 중심으로 선택한다.
- 거시환경 또는 리스크 요인과 해당 섹터의 연결 관계를 한 문장 이상 명확히 설명한다.
- 기존 포트폴리오의 편중을 어떻게 보완하는지 반드시 명시한다.
...
def _infer_portfolio_bias(tickers: List[str]) -> str:
    ...
    semiconductor_keywords = {"NVDA", "AMD", "TSM", "AVGO", "QCOM", "MU"}
    ...
    for t in tickers:
        if t in tech_keywords:
            counts["기술주"] += 1
        if t in semiconductor_keywords:
            counts["반도체"] += 1
        if t in bio_keywords:
            counts["헬스케어"] += 1
        if t in finance_keywords:
            counts["금융"] += 1
        if t in energy_keywords:
            counts["에너지"] += 1
...
def _build_sector_context(
    ...
    if any(keyword in macro_lower for keyword in ["ai", "인공지능", "클라우드", "디지털 전환"]):
        candidate_sectors.append(
            "- AI/반도체: 미국 대장주 후보는 NVIDIA, Microsoft, Broadcom, TSMC이며 한국 대장주 후보는 SK하이닉스, 삼성전자입니다."
        )
    ...
    if any(keyword in macro_lower for keyword in ["인프라", "전력", "전력망", "데이터센터", "전력 수요"]):
        candidate_sectors.append(
            "- 전력 인프라/유틸리티 장비: AI 데이터센터 확산과 전력 수요 증가는 전력 설비와 인프라 투자 확대 논리로 이어질 수 있습니다. 미국은 Eaton, GE Vernova, NextEra Energy, 한국은 LS ELECTRIC, 효성중공업 등을 참고할 수 있습니다."
        )

    if any(keyword in macro_lower for keyword in ["방어", "불확실", "변동성", "경기 둔화"]):
        candidate_sectors.append(
            "- 헬스케어: 경기 둔화나 변동성 확대 구간에서는 실적 방어력이 있는 제약·바이오 대형주가 대안이 될 수 있습니다. 미국은 Eli Lilly, Johnson & Johnson, Merck, 한국은 삼성바이오로직스, 셀트리온을 참고할 수 있습니다."
        )
    ...
    if any(keyword in portfolio_lower for keyword in ["기술주 편중", "tech", "성장주 편중", "나스닥 편중"]):
        candidate_sectors.append(
            "- 포트폴리오 분산 필요: 기술주 외 보완 섹터로 헬스케어, 전력 인프라, 산업재를 우선 검토할 수 있습니다."
        )

    # 기본값
    if not candidate_sectors:
        candidate_sectors = [
            "- 반도체/AI 인프라: 구조적 성장성과 실적 가시성을 동시에 점검할 수 있는 대표 섹터입니다. 미국은 NVIDIA, Microsoft, Broadcom, 한국은 SK하이닉스, 삼성전자입니다.",
            "- 헬스케어: 변동성 국면에서 방어력과 장기 성장성을 함께 고려할 수 있습니다. 미국은 Eli Lilly, Johnson & Johnson, Merck, 한국은 삼성바이오로직스, 셀트리온입니다.",
            "- 전력 인프라/산업재: AI 데이터센터와 전력 수요 확대 흐름의 수혜를 검토할 수 있습니다. 미국은 Eaton, GE Vernova, 한국은 LS ELECTRIC, 효성중공업입니다.",
        ]
